...
float EPSILON_NRM	= 0.1 / uScreenWidth;

const vec2 surfacePosition = vec2(1.0, 1.0);
...
#define time uTimer + length (surfacePosition)/sin(length (surfacePosition)*1e-1)
...
void main(void) {
    ...
    uv = uv * 2.0 - 1.0;	
    uv.x *= uScreenWidth / uScreenHeight;   
