...
cState loadState()
{
    ...
    state.angle =      texture2D(backbuffer,( vec2(1.4,0.0) / resolution.xy )).a * 6.28318;
    state.pos.x =      texture2D(backbuffer,( vec2(2.4,0.0) / resolution.xy )).a * 30.0;
    state.pos.y =      texture2D(backbuffer,( vec2(3.4,0.0) / resolution.xy )).a * 30.0;
...
void saveState( cState state, vec3 col )
{
    ...
    if( gl_FragCoord.y <= 0.5 )
    {
        if( gl_FragCoord.x <= 0.5 )
            alpha = 0.45;    //valid data
        else if( gl_FragCoord.x <= 1.5 )
            alpha = mod( state.angle / 6.28318, 1.0 );
        else if( gl_FragCoord.x <= 2.5 )
            alpha = mod( state.pos.x / 30.0, 1.0 );
        else if( gl_FragCoord.x <= 3.5 )
            alpha = mod( state.pos.y / 30.0, 1.0 );
    }
...
void main(void) {
	...
	vec3 dir = vec3( sin(state.angle), 0.0, cos(state.angle) );

	state.angle += ( mouse.x - 0.5 ) * 0.1;
	state.pos += dir.xz * ( mouse.y - 0.5 ) * 0.5;

	vec3 ro = vec3(35.0+state.pos.x,0.5,5.0+state.pos.y);
	vec3 ta = ro + dir;
	
    rdcenter = normalize( ta - ro);
