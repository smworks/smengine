...
float iGlobalTime = uTimer;
vec3 iResolution = vec3 (uScreenWidth, uScreenHeight, 0.0);

#define DELTA			0.01
#define RAY_LENGTH_MAX		150.0
#define RAY_STEP_MAX		200
#define LIGHT			vec3 (1.0, 3.0, 1.0)
#define AMBIENT			0.1
...
void main () {
	...
	vec4 backColor = vec4 (0.0, 0.0, 0.2 + 0.5 * max (0.0, dot (-direction, lightDirection)), 1.0);
	if (dist < DELTA) {
		...
		gl_FragColor = mix (backColor, ((AMBIENT + diffuse) * vec4 (0.8, 0.2, 0.8, 1.0) + specular), fade);
	}
	else {
