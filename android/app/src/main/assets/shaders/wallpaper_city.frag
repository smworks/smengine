...
// rotatation by Eular angle
vec3 rot(in vec3 x, in vec3 e)
{
	return vec3((cos(e.x)*cos(e.y)*cos(e.z)-sin(e.x)*sin(e.z))*x.x
		    -(cos(e.x)*cos(e.y)*sin(e.z)+sin(e.x)*cos(e.z))*x.y
		    +cos(e.x)*sin(e.y)*x.z,
		    (sin(e.x)*cos(e.y)*cos(e.z)+cos(e.x)*sin(e.z))*x.x
		    -(sin(e.x)*cos(e.y)*sin(e.z)-cos(e.x)*cos(e.z))*x.y
		    +sin(e.x)*sin(e.y)*x.z,
		    -sin(e.y)*cos(e.z)*x.x+sin(e.y)*sin(e.z)*x.y+cos(e.y)*x.z
		   );
}
...
void main(void){
	...
	E=vec3(-sin(uTimer/10.0)*10.0,5,cos(uTimer/10.0)*10.0); //spinning scene
	else if(CAMERA_MODE == MOUSE_CAMERA){
		float spin = 8.0; //uTimer * 0.1 + mouse.x * 8.0;
		E=vec3(-sin(spin)*10.0, 10.0, cos(spin)*10.0);//Change camera path position here
	}
	else if(CAMERA_MODE == PAN_CAMERA){
		E=vec3(-sin(1.0)*10.0,7,cos(1.0)*10.0);
		vec3 moveCamDir = normalize(vec3(E.x,0.0,E.y));
		float mouse_val = 0.5;
		E+=moveCamDir*uTimer*(mouse_val>0.0?mouse_val:0.0);
	}
	...
	if (totalDist<MAX_DEPTH){
		...
		if(SHADING_MODE==PHONG_SHADING){
			...
			if((fract(dist.y) >= 0.5)&&(fract(dist.y) < 1.5))
