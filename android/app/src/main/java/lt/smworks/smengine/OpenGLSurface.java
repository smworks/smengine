...
class OpenGLSurface extends GLSurfaceView {
	...
	private static class ContextFactory implements EGLContextFactory {
		...
		public EGLContext createContext(EGL10 egl, EGLDisplay display, EGLConfig eglConfig) {
			Log.d(MainActivity.TAG, "Creating OpenGL ES 2.0 context.");
		...
		public void destroyContext(EGL10 egl, EGLDisplay display,
				EGLContext context) {
			Log.d(MainActivity.TAG, "Destroying OpenGL ES 2.0 context.");
	...
	private static void checkEglError(String prompt, EGL10 egl) {
		...
		while ((error = egl.eglGetError()) != EGL10.EGL_SUCCESS) {
			Log.e(MainActivity.TAG, String.format("%s: EGL error: 0x%x", prompt, error));
		}
	}
		...
		public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
			for (EGLConfig config : configs) {
				...
				if (r == mRedSize && g == mGreenSize && b == mBlueSize && a == mAlphaSize) {
					Log.d(MainActivity.TAG, "Selected EGL config:");
		...
		private void printConfig(EGL10 egl, EGLDisplay display, EGLConfig config) {
			...
			for (int i = 0; i < attributes.length; i++) {
				...
				if (egl.eglGetConfigAttrib(display, config, attribute, value)) {
					Log.d(MainActivity.TAG, String.format("  %s: %d\n", name, value[0]));
				} else {
					Log.e(MainActivity.TAG, String.format("  %s: failed\n", name));
					int error = egl.eglGetError();
					if (error != EGL10.EGL_SUCCESS) {
						Log.e(MainActivity.TAG, String.format("    error: %d\n", error));
					}
				}
			}
		}
